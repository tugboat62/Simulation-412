...
# Total trials, Initial inventory level, number of months, number of policies
T, I, N, P = 0, 0, 0, 0
...
# Express_order setup and incremental cost
E_K, E_i = 0, 0

# Minlag and Maxlag periods in months
m, M = 0, 0

# Express_order minlag and maxlag
E_m, E_M = 0, 0

# cumulative probabilities of the sequential demand sizes
cum_prob = []

# Expected Express count
E_cnt = 0
...
def main():
    ...
    E_K, E_i = map(float, infile.readline().split())
    m, M = map(float, infile.readline().split())
    E_m, E_M = map(float, infile.readline().split())
    ...
    output_file.write(f"Total trials: {T}\n\n")
    ...
    output_file.write(f"Express order setup cost: {E_K}\n\n")
    output_file.write(f"Express order incremental cost: {E_i}\n\n")
    output_file.write(f"Delivery lag range: {m:.2f} to {M:.2f} months\n\n")
    output_file.write(f"Express order delivery lag range: {E_m:.2f} to {E_M:.2f} months\n\n")
    ...
    output_file.write(f"--------------------------------------------------------------------------------------------------\n")
    output_file.write(f"Policy\t\tAvg_total_cost\t\tExpected_express_count\n")
    output_file.write(f"--------------------------------------------------------------------------------------------------\n\n")
...
def evaluate():
    global inv_level, time_next_event, amount, total_ordering_cost, s, m, M, S, K, i, E_cnt 
    if inv_level < 0:
        E_cnt += 1
        amount = -inv_level
        total_ordering_cost += E_K + E_i * amount
...
def report():
    global total_ordering_cost, area_holding, area_shortage, N, h, pi, s, S, out_file, T

    # Compute and write estimates of desired measures of performance.
    avg_holding_cost = h * area_holding / (N*T)
    avg_ordering_cost = total_ordering_cost / (N*T)
    avg_shortage_cost = pi * area_shortage / (N*T)
    avg_exp = E_cnt / T
